...
export default function FoodCard(dish: any) {
  const router = useRouter();
  const { addItem } = useCart();

  const handleNavigate = (e: React.MouseEvent) => {
    e.stopPropagation();
    router.push(`/items/${dish.id}`);
  };
  ...
  return <Card key={dish.id} className="overflow-hidden card-hover border-gray-300" onClick={handleNavigate}>
